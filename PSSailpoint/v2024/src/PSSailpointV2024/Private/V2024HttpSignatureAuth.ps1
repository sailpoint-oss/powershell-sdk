...
.OUTPUTS
    Hashtable
#>
function Get-V2024HttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-V2024ConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-V2024StringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-V2024UnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-V2024UnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-V2024StringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-V2024KeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-V2024RSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-V2024ECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-V2024CryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-V2024RSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "V2024RSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-V2024ECDSASignature {
    param(
...
function Get-V2024CryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-V2024KeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
