...
AutomatedWorkflow<PSCustomObject>
#>

function Initialize-NERMAutomatedWorkflow {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSSailpoint.NERM => NERMAutomatedWorkflow' | Write-Debug
        ...
        if (!$ConditionRulesAttributes) {
            throw "invalid value for 'ConditionRulesAttributes', 'ConditionRulesAttributes' cannot be null."
        }
...
Convert from JSON to AutomatedWorkflow<PSCustomObject>
...
AutomatedWorkflow<PSCustomObject>
#>
function ConvertFrom-NERMJsonToAutomatedWorkflow {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSSailpoint.NERM => NERMAutomatedWorkflow' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in NERMAutomatedWorkflow
        $AllProperties = ("profile_type_id", "status", "uid", "name", "disable_failure_email_notifications", "condition_rules_attributes")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "condition_rules_attributes"))) {
            throw "Error! JSON cannot be serialized due to the required property 'condition_rules_attributes' missing."
        } else {
