...
function Initialize-V2026LoadAccountsTaskTaskAttributes {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSSailpoint.V2026 => V2026LoadAccountsTaskTaskAttributes' | Write-Debug
...
LoadAccountsTaskTaskAttributes<PSCustomObject>
#>
function ConvertFrom-V2026JsonToLoadAccountsTaskTaskAttributes {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSSailpoint.V2026 => V2026LoadAccountsTaskTaskAttributes' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json
        $V2026LoadAccountsTaskTaskAttributesAdditionalProperties = @{}

        # check if Json contains properties not defined in V2026LoadAccountsTaskTaskAttributes
        $AllProperties = ("appId", "optimizedAggregation")
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            # store undefined properties in additionalProperties
            if (!($AllProperties.Contains($name))) {
                $V2026LoadAccountsTaskTaskAttributesAdditionalProperties[$name] = $JsonParameters.PSobject.Properties[$name].value
            }
        }
        ...
        $PSO = [PSCustomObject]@{
            "appId" = ${AppId}
            "optimizedAggregation" = ${OptimizedAggregation}
            "AdditionalProperties" = $V2026LoadAccountsTaskTaskAttributesAdditionalProperties
        }
