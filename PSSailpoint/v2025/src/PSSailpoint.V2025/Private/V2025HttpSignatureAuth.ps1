...
.OUTPUTS
    Hashtable
#>
function Get-V2025HttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-V2025ConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-V2025StringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-V2025UnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-V2025UnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-V2025StringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-V2025KeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-V2025RSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-V2025ECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-V2025CryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-V2025RSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "V2025RSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-V2025ECDSASignature {
    param(
...
function Get-V2025CryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-V2025KeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
