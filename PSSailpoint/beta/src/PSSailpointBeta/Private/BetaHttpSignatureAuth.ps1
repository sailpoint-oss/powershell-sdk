...
.OUTPUTS
    Hashtable
#>
function Get-BetaHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-BetaConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-BetaStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-BetaUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-BetaUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-BetaStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-BetaKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-BetaRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-BetaECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
...
function Get-BetaRSASignature {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "BetaRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-BetaECDSASignature {
    param(
...
function Get-BetaCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-BetaKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
