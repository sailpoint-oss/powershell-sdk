...
.OUTPUTS
    Hashtable
#>
function Get-CCHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-CCConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-CCStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-CCUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-CCUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-CCStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-CCKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-CCRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-CCECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-CCCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
function Get-CCRSASignature {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "CCRSAEncryptionProvider.cs"
...
String
#>
function Get-CCKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
