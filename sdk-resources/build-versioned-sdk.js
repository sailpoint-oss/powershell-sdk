...
function writePartitionConfig(partitionName) {
  ...
  const packageName   = `PSSailpoint.${pascal}V1`;
...
function generatePartition(partitionName, bundledSpec, configPath) {
  const outputDir = path.join(SDK_ROOT, "PSSailpoint", `${partitionName}_v1`);
...
function generateBarrelModule() {
  ...
  const partitionDirs = fs.readdirSync(psSailpointDir, { withFileTypes: true })
    .filter(e => e.isDirectory() && /^[a-z].+_v\d{1,2}$/.test(e.name))
    .map(e => e.name)
    .sort();

  if (partitionDirs.length === 0) {
    console.log("  No partition_v{n} modules found, skipping barrel module generation.");
    return;
  }

  const importLines = partitionDirs.map(dir => {
    const version  = dir.match(/_v(\d+)$/)[1];
    const baseName = dir.replace(/_v\d+$/, "");
    const pascal   = toPascal(baseName);
    const modName  = `PSSailpoint.${pascal}V${version}`;
    return `Import-Module -Name (Join-Path $PSScriptRoot '${dir}' 'src' '${modName}') -Force -Global`;
  });
...
async function main() {
  ...
  for (const partition of partitions) {
    ...
    console.log(`  ✓ ${partition} → PSSailpoint/${partition}_v1/`);
  }
