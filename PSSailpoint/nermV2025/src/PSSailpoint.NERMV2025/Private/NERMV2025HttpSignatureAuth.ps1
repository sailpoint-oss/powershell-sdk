...
.OUTPUTS
    Hashtable
#>
function Get-NERMV2025HttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-NERMV2025ConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-NERMV2025StringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-NERMV2025UnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-NERMV2025UnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-NERMV2025StringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-NERMV2025KeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-NERMV2025RSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-NERMV2025ECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-NERMV2025CryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-NERMV2025RSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "NERMV2025RSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-NERMV2025ECDSASignature {
    param(
...
.Outputs
    String
#>
Function Get-NERMV2025StringHash {
    param(
        [Parameter(Mandatory = $true)]
        [AllowEmptyString()]
        [string]$String,
...
Integer
#>
function Get-NERMV2025UnixTime {
    param(
        [Parameter(Mandatory = $true)]
        [DateTime]$Date,
        [Parameter(Mandatory = $false)]
...
function Get-NERMV2025CryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-NERMV2025KeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
