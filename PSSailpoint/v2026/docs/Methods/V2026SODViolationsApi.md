...
keywords: ['powershell', 'PowerShell', 'sdk', 'SODViolations', 'V2026SODViolations'] 
slug: /tools/sdk/powershell/v2026/methods/sod-violations
...
------------- | ------------- | -------------
[**Start-V2026PredictSodViolations**](#start-predict-sod-violations) | **POST** `/sod-violations/predict` | Predict sod violations for identity.
[**Start-V2026ViolationCheck**](#start-violation-check) | **POST** `/sod-violations/check` | Check sod violations
...
try {
    $Result = ConvertFrom-V2026JsonToIdentityWithNewAccess -Json $IdentityWithNewAccess
    Start-V2026PredictSodViolations -IdentityWithNewAccess $Result 
    
    # Below is a request that includes all optional parameters
    # Start-V2026PredictSodViolations -IdentityWithNewAccess $Result  
} catch {
    Write-Host $_.Exception.Response.StatusCode.value__ "Exception occurred when calling Start-V2026PredictSodViolations"
...
try {
    $Result = ConvertFrom-V2026JsonToIdentityWithNewAccess -Json $IdentityWithNewAccess
    Start-V2026ViolationCheck -IdentityWithNewAccess $Result 
    
    # Below is a request that includes all optional parameters
    # Start-V2026ViolationCheck -IdentityWithNewAccess $Result  
} catch {
    Write-Host $_.Exception.Response.StatusCode.value__ "Exception occurred when calling Start-V2026ViolationCheck"
    Write-Host $_.ErrorDetails
}
```
[[Back to top]](#) 
