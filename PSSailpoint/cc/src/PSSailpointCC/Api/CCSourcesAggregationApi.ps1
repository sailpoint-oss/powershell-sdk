...
System.Collections.Hashtable
#>
function Invoke-CCLoadEntitlements {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Mandatory = $false)]
        [String]
    ...
    Process {
        'Calling method: Invoke-CCLoadEntitlements' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug
        ...
        $LocalVarUri = '/cc/api/source/loadEntitlements/{id}'
        if (!$Id) {
            throw "Error! The required parameter `Id` missing when calling loadEntitlements."
        }
        $LocalVarUri = $LocalVarUri.replace('{id}', [System.Web.HTTPUtility]::UrlEncode($Id))
