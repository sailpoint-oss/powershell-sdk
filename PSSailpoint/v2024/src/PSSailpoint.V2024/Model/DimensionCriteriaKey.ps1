#
# Identity Security Cloud V2024 API
# Use these APIs to interact with the Identity Security Cloud platform to achieve repeatable, automated processes with greater scalability. We encourage you to join the SailPoint Developer Community forum at https://developer.sailpoint.com/discuss to connect with other developers using our APIs.
# Version: v2024
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

Refers to a specific Identity attribute used in Dimension membership criteria.

.PARAMETER Type
No description available.
.PARAMETER Property
The name of the identity attribute to which the associated criteria applies.
.OUTPUTS

DimensionCriteriaKey<PSCustomObject>
#>

function Initialize-V2024DimensionCriteriaKey {
    [CmdletBinding()]
    Param (
        [Parameter(ValueFromPipelineByPropertyName = $true)]
        [ValidateSet("IDENTITY")]
        [PSCustomObject]
        ${Type},
        [Parameter(ValueFromPipelineByPropertyName = $true)]
        [String]
        ${Property}
    )

    Process {
        'Creating PSCustomObject: PSSailpoint.V2024 => V2024DimensionCriteriaKey' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if (!$Type) {
            throw "invalid value for 'Type', 'Type' cannot be null."
        }

        if (!$Property) {
            throw "invalid value for 'Property', 'Property' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            "type" = ${Type}
            "property" = ${Property}
        }

        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to DimensionCriteriaKey<PSCustomObject>

.DESCRIPTION

Convert from JSON to DimensionCriteriaKey<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

DimensionCriteriaKey<PSCustomObject>
#>
function ConvertFrom-V2024JsonToDimensionCriteriaKey {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: PSSailpoint.V2024 => V2024DimensionCriteriaKey' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in V2024DimensionCriteriaKey
        $AllProperties = ("type", "property")
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'type' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "type"))) {
            throw "Error! JSON cannot be serialized due to the required property 'type' missing."
        } else {
            $Type = $JsonParameters.PSobject.Properties["type"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "property"))) {
            throw "Error! JSON cannot be serialized due to the required property 'property' missing."
        } else {
            $Property = $JsonParameters.PSobject.Properties["property"].value
        }

        $PSO = [PSCustomObject]@{
            "type" = ${Type}
            "property" = ${Property}
        }

        return $PSO
    }

}

