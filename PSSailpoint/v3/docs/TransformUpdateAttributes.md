...
**IgnoreErrors** | **Boolean** | a true or false value representing to move on to the next option if an error (like an Null Pointer Exception) were to occur. | [optional] 
...
```powershell
$TransformUpdateAttributes = Initialize-PSSailpointTransformUpdateAttributes  -SourceName Workday `
