...
keywords: ['powershell', 'PowerShell', 'sdk', 'ApplicationDiscovery', 'V2026ApplicationDiscovery'] 
slug: /tools/sdk/powershell/v2026/methods/application-discovery
...
------------- | ------------- | -------------
[**Get-V2026DiscoveredApplications**](#get-discovered-applications) | **GET** `/discovered-applications` | Get discovered applications for tenant
[**Get-V2026ManualDiscoverApplicationsCsvTemplate**](#get-manual-discover-applications-csv-template) | **GET** `/manual-discover-applications-template` | Download csv template for discovery
[**Send-V2026ManualDiscoverApplicationsCsvTemplate**](#send-manual-discover-applications-csv-template) | **POST** `/manual-discover-applications` | Upload csv to discover applications
[**Start-V2026ApplicationDiscovery**](#start-application-discovery) | **POST** `/sources/{sourceId}/discover-applications` | Start Application Discovery
...
try {
    Get-V2026DiscoveredApplications 
    
    # Below is a request that includes all optional parameters
    # Get-V2026DiscoveredApplications -Limit $Limit -Offset $Offset -Detail $Detail -Filter $Filter -Sorters $Sorters  
} catch {
    Write-Host $_.Exception.Response.StatusCode.value__ "Exception occurred when calling Get-V2026DiscoveredApplications"
...
try {
    ...
    Write-Host $_.Exception.Response.StatusCode.value__ "Exception occurred when calling Get-V2026ManualDiscoverApplicationsCsvTemplate"
    Write-Host $_.ErrorDetails
}
```
[[Back to top]](#) 
...
try {
    Send-V2026ManualDiscoverApplicationsCsvTemplate -File $File 
    
    # Below is a request that includes all optional parameters
    # Send-V2026ManualDiscoverApplicationsCsvTemplate -File $File  
} catch {
    Write-Host $_.Exception.Response.StatusCode.value__ "Exception occurred when calling Send-V2026ManualDiscoverApplicationsCsvTemplate"
    Write-Host $_.ErrorDetails
}
```
[[Back to top]](#) 
...
try {
    $Result = ConvertFrom-V2026JsonToApplicationDiscoveryRequest -Json $ApplicationDiscoveryRequest
    Start-V2026ApplicationDiscovery -XSailPointExperimental $XSailPointExperimental -ApplicationDiscoveryRequest $Result 
    
    # Below is a request that includes all optional parameters
    # Start-V2026ApplicationDiscovery -XSailPointExperimental $XSailPointExperimental -ApplicationDiscoveryRequest $Result  
} catch {
    Write-Host $_.Exception.Response.StatusCode.value__ "Exception occurred when calling Start-V2026ApplicationDiscovery"
    Write-Host $_.ErrorDetails
}
```
[[Back to top]](#) 
